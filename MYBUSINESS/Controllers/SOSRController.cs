    ...
    public class SOSRController : Controller
    {
        ...
        private void GetTotalBalance(ref IQueryable<SO> SOes)
        {
            ...
            foreach (SO itm in DistSOes)
            {
                ...
                if (cust != null && cust.Balance != null)
                {
        ...
        public ActionResult Create(string IsReturn)
        {
            ...
            ViewBag.ReportId = TempData["SOSRID"] as string;
        ...
        public ActionResult Create([Bind(Prefix = "Customer", Include = "Name,Address")] Customer Customer, [Bind(Prefix = "SaleOrder", Include = "BillAmount,Balance,PrevBalance,BillPaid,Discount,CustomerId,Remarks,Remarks2,PaymentMethod,PaymentDetail,SaleReturn,BankAccountId,Date,StockType")] SO sO, [Bind(Prefix = "SaleOrderDetail", Include = "ProductId,SalePrice,Quantity,SaleType,PerPack,IsPack,Product")] List<SOD> sOD, FormCollection collection)
                    ...
                    foreach (SOD sod in sOD)
                    {
                        ...
                        else//return
                        {
                            if (sod.IsPack == false)
                            {
                                sO.SaleOrderAmount += (sod.Quantity * sod.SalePrice);
                                decimal qty = (decimal)sod.Quantity;// / (decimal)product.PerPack;
                                if (sO.StockType == 1)//main stock
                                {
                                    product.Stock += qty;
                                }
                                else//d.stock
                                {
                                    product.StockDefective += (int)qty;
                                }
                                sO.SaleOrderQty += qty;//(int)sod.Quantity;
                                sO.Profit += (qty * sod.SalePrice) - (decimal)(qty * product.PurchasePrice); //- (decimal)(sO.Discount);
                            }
                            else
                            {
                                ...
                                {
                                    product.StockDefective += (int)(sod.Quantity * sod.PerPack);
                                }
                        ...
                        //SubItemStockMaintenance(sOD, newProd, sno, sO);



                        //subItems = db.SubItems.Where(x => x.ParentProductId == sod.ProductId).ToList();
                        //foreach (SubItem subItem in subItems)
                        //{
                        //    if (sod.SaleType == false)//sale
                        //    {
                        //        subItem.Product.Stock -= subItem.Quantity * sod.Quantity;
                        //        //remaining = Convert.ToDecimal( subItem.Product.Stock - subItem.Quantity);
                        //    }
                        //    else//return
                        //    {
                        //        subItem.Product.Stock += subItem.Quantity * sod.Quantity;
                        //    }
                        //    db.Products.Attach(subItem.Product);
                        //    db.Entry(subItem.Product).Property(x => x.Stock).IsModified = true;
                        //}
        ...
        //private void StockMaintenance(List<SOD> sOD, Product newProd, int sno, SO sO){}
        private void StockMaintenance(List<SOD> sOD, Product newProd, int sno, SO sO)
        {
        }
        ...
        public ActionResult Edit(string id, bool update)
        {
            ...
            List<StockType> StockTypes = new List<StockType> {
                            ...
                            new StockType {
                                Text = "Defective Stock",
                                Value = "2"
                            }
                        };
            ViewBag.StockTypes = StockTypes;
